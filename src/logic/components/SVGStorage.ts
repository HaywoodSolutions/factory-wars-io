 ...
 * @author Spedwards
 */
export default class SVGStorage extends SVGStorageObject {
	/**
	 * @author Spedwards
	 ...
	 */
	constructor(storage: Storage, protected size = 60) {
	...
	toString() {
		if (!this.string) {
			...
			const TOTAL = _.sum(Object.values(STORAGE.store));
	...
	static getSVG(storage: Storage, size?: number) {
		return new SVGStorage(storage, size).toString();
	}
}