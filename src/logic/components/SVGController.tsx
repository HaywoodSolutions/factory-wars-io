 ...
 * @author Spedwards
 */
export default class SVGController extends SVG {
	/**
	 * @author Spedwards
	 ...
	 */
	constructor(private controller: Controller, protected size: number = 60) {
		super();
	...
	toString() {
		if (!this.string) {
			...
			if (this.controller.level > 0 && this.controller.progress > 0) {
				...
				const CONTROLLER_DOWNGRADE: { [level: number]: number } = {};
	...
	static getSVG(controller: Controller, size?: number): string {
		return new SVGController(controller, size).toString();
	}
	...
	render(): JSX.Element {
		...
		let out = (
			<svg height={SVG_SIZE} width={SVG_SIZE} viewBox="0 0 300 300">
				<g transform="translate(150,150)">
					<g>
						<>
							<path fill="#FFFFFF" fill-opacity="0.1" transform="scale(1.2 1.2)" d={`M ${START_X} ${START_Y} L ${DEST} Z`} />
							{this.controller.reservation && this.player === this.controller.reservation.username &&
								<ellipse cx="0" cy="0" fill="#33FF33" opacity="0" rx="110" ry="110">
									<animate attributeName="opacity" attributeType="XML" dur="2s" repeatCount="indefinite" values="0.05;0.2;0.05" calcMode="linear" />
								</ellipse>
							}
							{this.controller.upgradeBlocked || this.controller.reservation && !(this.player === this.controller.reservation.username) &&
								<ellipse cx="0" cy="0" fill="#FF3333" opacity="0" rx="110" ry="110">
									<animate attributeName="opacity" attributeType="XML" dur="2s" repeatCount="indefinite" values="0.05;0.2;0.05" calcMode="linear" />
								</ellipse>
							}
							{this.controller.safeMode &&
								<ellipse cx="0" cy="0" fill="#FFD180" opacity="0" rx="110" ry="110">
									<animate attributeName="opacity" attributeType="XML" dur="2s" repeatCount="indefinite" values="0.05;0.2;0.05" calcMode="linear" />
								</ellipse>
							}
							<path fill="#0A0A0A" d={`M ${START_X} ${START_Y} L ${DEST} Z`} />
							{this.controller.level &&
								<path fill="#CCCCCC" paint-order="fill" strokeWidth="6" stroke="#0A0A0A" d={this.getLevelsPath()} />
							}
							<ellipse cx="0" cy="0" fill="#222222" rx="37" ry="37" />
							<ellipse cx="0" cy="0" fill="transparent" rx="40" ry="40" strokeWidth="10" stroke="#080808"></ellipse>
							{
								this.controller.level > 0 && this.controller.progress > 0 && (() => {
									const LARGE_ARC_FLAG = this.controller.progress < this.controller.progressTotal / 2 ? 0 : 1;
									const END_X = 19 * Math.cos(-Math.PI * 2 * (this.controller.progressTotal - this.controller.progress) / this.controller.progressTotal);
									const END_Y = 19 * Math.sin(-Math.PI * 2 * (this.controller.progressTotal - this.controller.progress) / this.controller.progressTotal);
									...
									return (
										<>
											<path fill="transparent" stroke-opacity="0.4" strokeWidth="38" stroke="#FFFFFF" transform="rotate(-90)" d={`M 19 0 A 19 19 0 ${LARGE_ARC_FLAG} 1 ${END_X} ${END_Y}`} />
											<g opacity={DOWNGRADE_OPACITY}>
												<path className="anim-downgrade" fill="#FF3333" d={`M ${START_X} ${START_Y} L ${DEST} Z`}>
													<animate attributeName="opacity" attributeType="XML" repeatCount="indefinite" values="0;0.9;0.6;0.3;0" dur="2s" calcMode="linear" />
												</path>
											</g>
										</>
