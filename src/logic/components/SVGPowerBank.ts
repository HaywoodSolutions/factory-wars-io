 ...
 * @author Spedwards
 */
export default class SVGPowerBank extends SVG {
	/**
	 * @author Spedwards
	 ...
	 */
	constructor(private powerBank: PowerBank, protected size: number = 50) {
		super();
	...
	static getSVG(powerBank: PowerBank, size?: number): string {
		return new SVGPowerBank(powerBank, size).toString();
	}
};
