import SVG from './SVG';
import SVGStorage, { Storage } from './SVGStorage';
import SVGTerminal, { Terminal } from './SVGTerminal';
 ...
 * @author Spedwards
 */
export default class SVGRoom extends SVG {
	/**
	 * @author Spedwards
	 * @param {Room | string} roomArg - Room object or valid room name.
	 * @param {Number} [size = 60] - SVG size.
	 */
	constructor(private room: Room, protected size: number = 60) {
		super();
	...
	static getSVG(room: Room, size?: number): string {
		return new SVGRoom(room, size).toString();
	}
};
