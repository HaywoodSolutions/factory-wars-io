import SVGStorageObject, { StorageObject } from './SVGStorageObject';
import _ from 'lodash';

export type Terminal = StorageObject;

/**
 * Returns a html/svg string representation of the given terminal object.
 * @author Spedwards
 */
export default class SVGTerminal extends SVGStorageObject {
	/**
	 * @author Spedwards
	 ...
	 */
	constructor(terminal: Terminal, protected size: number = 60) {
		super(terminal, "STRUCTURE_STORAGE");
	...
	toString() {
		if (!this.string) {
			...
			const TOTAL = _.sum(Object.values(TERMINAL.store));
	...
	static getSVG(terminal: Terminal, size?: number) {
		return new SVGTerminal(terminal, size).toString();
	}
}