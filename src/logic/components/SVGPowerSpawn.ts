 ...
 * @author Spedwards
 */
export default class SVGPowerSpawn extends SVG {
	/**
	 * @author Spedwards
	 ...
	 */
	constructor(private powerSpawn: PowerSpawn, protected size: number = 50) {
		super();
	...
	static getSVG(powerSpawn: PowerSpawn, size?: number): string {
		return new SVGPowerSpawn(powerSpawn, size).toString();
	}
};
