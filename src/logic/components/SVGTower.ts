...
export default class SVGTower extends SVG {
	 ...
	 */
	constructor(private tower: Tower, angle: number = 315, animated: boolean = false, protected size: number = 60) {
		super();
	...
	static getSVG(tower: Tower, angle: number = 315, animated: boolean = false, size?: number): string {
		return new SVGTower(tower, angle, animated, size).toString();
	}
}